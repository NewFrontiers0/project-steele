...
HTTP firmware serving uses a named streaming profile. The default `balanced` profile now uses a short 64 KB slow-start phase, then accelerates the stream with 64 KB writes, normal MSS, a bounded send buffer, and TCP_NOTSENT_LOWAT where Linux supports it. This avoids the original immediate VM burst that IOS-XE rejected, but removes the per-chunk sleep for most of the image. If a switch regresses, set `SWIM_HTTP_PROFILE=safe` in `/opt/project-steele/.env` to roll back to the original 512-byte / 5 ms profile, then restart the service. Use `SWIM_HTTP_PROFILE=fast` only on clean lab networks. Use `SWIM_HTTP_PROFILE=custom` if you want `SWIM_HTTP_CHUNK_BYTES`, `SWIM_HTTP_CHUNK_DELAY_MS`, `SWIM_HTTP_ACCELERATE_AFTER_BYTES`, `SWIM_HTTP_ACCELERATED_CHUNK_BYTES`, `SWIM_HTTP_ACCELERATED_CHUNK_DELAY_MS`, `SWIM_HTTP_INITIAL_DELAY_MS`, `SWIM_HTTP_TCP_MAXSEG`, `SWIM_HTTP_SNDBUF_BYTES`, and `SWIM_HTTP_TCP_NOTSENT_LOWAT` to be read directly.
